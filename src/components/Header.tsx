      ...
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-3">
          <Link href="/" className="flex items-center space-x-2 transition-transform hover:-translate-y-0.5 duration-200">
            <Image src="https://placehold.co/160x40/16a34a/FFFFFF?text=Logo+Desa" alt="Logo Desa Kedungwungu" width={160} height={40} className="h-10 w-auto"/>
          </Link>

          <div className="hidden lg:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.href} className={`text-sm font-medium pb-1 ${link.href === '/' ? 'text-emerald-600 border-b-2 border-emerald-500' : 'text-gray-600 hover:text-emerald-600 border-b-2 border-transparent hover:border-emerald-300 transition-all'}`}>
                {link.name}
              </Link>
            ))}
          </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-full inset-x-0 p-2">
          <div className="bg-white rounded-lg shadow-lg p-4 space-y-2">
            {navLinks.map((link) => (
               <Link key={link.name} href={link.href} className={`block px-4 py-2 text-base font-medium rounded-md ${link.href === '/' ? 'text-emerald-700 bg-emerald-50' : 'text-gray-600 hover:bg-gray-50 hover:text-emerald-700'}`} onClick={() => setMobileMenuOpen(false)}>
                {link.name}
              </Link>
            ))}
